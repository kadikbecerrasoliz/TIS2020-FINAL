<div class="modal-header text-center">
    <h5 class="modal-title w-100" id="exampleModalLabel">Subir Detalle</h5>
...
<div class="modal-body">
    <form action="{{ route('certificados.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div class="col">
                <label for="roles">Detalle</label>
                <select class="browser-default custom-select" id="detalle_id" name="detalle_id">
                    <option value="{{$detalle->id}}"> {{$detalle->titulo}} </option>
                </select>
            </div>
        </div>
        <div class="form-row">
